...
export default function DeleteIdeaModal({
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/ideas/${ideaSlug}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        ...
        <p className="text-gray-600 mb-4">
          Are you sure you want to delete <strong>{ideaName}</strong>? This
          will permanently remove the idea, all its companies, and all survey
